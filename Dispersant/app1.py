...
def simulate_efficiency(days, initial_temperature, wind_speed, wave_length, base_efficiency):
    ...
    for day in range(days):
        ...
        efficiency = base_efficiency * (1.01 ** temp_factor) * (0.98 ** wind_factor) * (0.97 ** wave_factor) * (0.95 ** day)
...
if st.sidebar.button('Прогноз'):
    ...
    if base_efficiency < 50:
        ...
        st.info(f'У вас есть в запасе {reserve_days - 1} дней, прежде чем диспергент будет не эффективно.')
        ...
        ax.text(reserve_days - 1, 50, 'Optimum Reserve Days', color='red', ha='center', va='bottom')
